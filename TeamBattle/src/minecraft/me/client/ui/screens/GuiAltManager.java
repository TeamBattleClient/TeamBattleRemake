...
import me.client.Threads.AltLoginThread;
import me.client.alts.Alt;
import me.client.helpers.RenderHelper;
...
public class GuiAltManager extends GuiScreen {
	...
	public GuiAltManager() {
		Client.getAltManager().getContents().clear();
	...
	public void actionPerformed(GuiButton button) {
		...
		case 2:
			...
			Client.getAltManager().getContents().remove(selectedAlt);
			status = "\247aRemoved.";
			selectedAlt = null;
		...
		case 4:
			...
			break;
		case 5:
			final Alt randomAlt = Client
					.getAltManager()
					.getContents()
					.get(new Random().nextInt(Client.getAltManager()
							.getContents().size()));
	...
	public void drawScreen(int par1, int par2, float par3) {
		...
		drawCenteredString(fontRendererObj, "Account Manager - "
				+ Client.getAltManager().getContents().size() + " alts",
		...
		int y = 38;
		for (final Alt alt : Client.getAltManager().getContents()) {
			if (!isAltInArea(y)) {
				continue;
			}
			String name, pass;
