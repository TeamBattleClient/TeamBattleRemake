...
public class BlockHelper implements MinecraftHelper  {
	...
	public static float[] getBlockRotations(int x, int y, int z) {
		...
		final double var8 = y - (mc.thePlayer.posY + mc.thePlayer.getEyeHeight() - 1.0D);
	...
	public static boolean isOnIce() {
		boolean onIce = false;
		final int y = (int) mc.thePlayer.boundingBox.copy().offset(0.0D, -0.1D,0.0D).minY;
		for (int x = MathHelper.floor_double(mc.thePlayer.boundingBox.minX); x < MathHelper.floor_double(mc.thePlayer.boundingBox.maxX) + 1; x++) {
			for (int z = MathHelper.floor_double(mc.thePlayer.boundingBox.minZ); z < MathHelper.floor_double(mc.thePlayer.boundingBox.maxZ) + 1; z++) {
				final Block block = mc.theWorld.getBlock(x, y, z);
				if (block != null && !(block instanceof BlockAir)) {
	...
	public static boolean isOnLadder() {
		...
		final int y = (int) mc.thePlayer.boundingBox.copy().offset(0.0D, 1.0D, 0.0D).minY;
		for (int x = MathHelper.floor_double(mc.thePlayer.boundingBox.minX); x < MathHelper	.floor_double(mc.thePlayer.boundingBox.maxX) + 1; x++) {
			for (int z = MathHelper.floor_double(mc.thePlayer.boundingBox.minZ); z < MathHelper.floor_double(mc.thePlayer.boundingBox.maxZ) + 1; z++) {
				final Block block = mc.theWorld.getBlock(x, y, z);
				if (block != null && !(block instanceof BlockAir)) {
	...
	public static boolean isOnLiquid() {
		...
		final int y = (int) mc.thePlayer.boundingBox.copy().offset(0.0D,-0.01D, 0.0D).minY;
		for (int x = MathHelper.floor_double(mc.thePlayer.boundingBox.minX); x < MathHelper.floor_double(mc.thePlayer.boundingBox.maxX) + 1; x++) {
			for (int z = MathHelper.floor_double(mc.thePlayer.boundingBox.minZ); z < MathHelper.floor_double(mc.thePlayer.boundingBox.maxZ) + 1; z++) {
				final Block block = mc.theWorld.getBlock(x, y, z);
				if (block != null && !(block instanceof BlockAir)) {
