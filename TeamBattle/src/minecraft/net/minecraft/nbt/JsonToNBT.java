			...
			try {
				if (field_150493_b.matches("[-+]?[0-9]*\\.?[0-9]+[d|D]"))
					return new NBTTagDouble(Double.parseDouble(field_150493_b
							.substring(0, field_150493_b.length() - 1)));
				else if (field_150493_b.matches("[-+]?[0-9]*\\.?[0-9]+[f|F]"))
					return new NBTTagFloat(Float.parseFloat(field_150493_b
							.substring(0, field_150493_b.length() - 1)));
				else if (field_150493_b.matches("[-+]?[0-9]+[b|B]"))
					return new NBTTagByte(Byte.parseByte(field_150493_b
							.substring(0, field_150493_b.length() - 1)));
				else if (field_150493_b.matches("[-+]?[0-9]+[l|L]"))
					return new NBTTagLong(Long.parseLong(field_150493_b
							.substring(0, field_150493_b.length() - 1)));
				else if (field_150493_b.matches("[-+]?[0-9]+[s|S]"))
					return new NBTTagShort(Short.parseShort(field_150493_b
							.substring(0, field_150493_b.length() - 1)));
				else if (field_150493_b.matches("[-+]?[0-9]+"))
					return new NBTTagInt(Integer.parseInt(field_150493_b
							.substring(0, field_150493_b.length())));
				else if (field_150493_b.matches("[-+]?[0-9]*\\.?[0-9]+"))
					return new NBTTagDouble(Double.parseDouble(field_150493_b
							.substring(0, field_150493_b.length())));
				else if (!field_150493_b.equalsIgnoreCase("true")
						&& !field_150493_b.equalsIgnoreCase("false")) {
					if (field_150493_b.startsWith("[")
							&& field_150493_b.endsWith("]")) {
						if (field_150493_b.length() > 2) {
							final String var1 = field_150493_b.substring(1,
									field_150493_b.length() - 1);
							final String[] var2 = var1.split(",");

							try {
								if (var2.length <= 1)
									return new NBTTagIntArray(
											new int[] { Integer.parseInt(var1
													.trim()) });
								else {
						...
						if (field_150493_b.startsWith("\"")
								&& field_150493_b.endsWith("\"")
								&& field_150493_b.length() > 2) {
							field_150493_b = field_150493_b.substring(1,
									field_150493_b.length() - 1);
						}
					...
					return new NBTTagByte(
							(byte) (Boolean.parseBoolean(field_150493_b) ? 1
									: 0));
		...
		for (int var4 = 0; var4 < p_150310_0_.length(); ++var4) {
			...
			if (var5 == 34) {
				if (var4 > 0 && p_150310_0_.charAt(var4 - 1) == 92) {
					if (!var2)
						throw new NBTException("Illegal use of \\\": "
								+ p_150310_0_);
				} else {
					var2 = !var2;
				}
				...
				if (var5 != 123 && var5 != 91) {
					if (var5 == 125
							&& (var3.isEmpty() || ((Character) var3.pop())
									.charValue() != 123))
						throw new NBTException("Unbalanced curly brackets {}: "
								+ p_150310_0_);
					...
					if (var5 == 93
						...
						throw new NBTException(
								"Unbalanced square brackets []: " + p_150310_0_);
				} else {
					if (var3.isEmpty()) {
						++var1;
					}
	...
	private static String func_150311_c(String p_150311_0_, boolean p_150311_1_)
		...
		if (var2 < 0) {
			if (p_150311_1_)
				...
				throw new NBTException(
						"Unable to locate name/value separator for string: "
								+ p_150311_0_);
		} else
	...
	public static NBTBase func_150315_a(String p_150315_0_) throws NBTException {
		...
		if (var1 != 1)
			...
			if (p_150315_0_.startsWith("{")) {
				...
				var2 = func_150316_a(func_150313_b(p_150315_0_, false),
	...
	static JsonToNBT.Any func_150316_a(String p_150316_0_, String p_150316_1_)
		...
		if (p_150316_1_.startsWith("{")) {
			if (!p_150316_1_.endsWith("}"))
				throw new NBTException("Unable to locate ending bracket for: "
						+ p_150316_1_);
			else {
				p_150316_1_ = p_150316_1_
						.substring(1, p_150316_1_.length() - 1);
				final JsonToNBT.Compound var7 = new JsonToNBT.Compound(
						p_150316_0_);
				...
				while (p_150316_1_.length() > 0) {
					...
					if (var3.length() > 0) {
						...
						if (var6 != 44 && var6 != 123 && var6 != 125
								&& var6 != 91 && var6 != 93)
							throw new NBTException("Unexpected token \'" + var6
									+ "\' at: "
									+ p_150316_1_.substring(var3.length()));
		...
		} else if (p_150316_1_.startsWith("[")
				&& !p_150316_1_.matches("\\[[-\\d|,\\s]+\\]")) {
			if (!p_150316_1_.endsWith("]"))
				throw new NBTException("Unable to locate ending bracket for: "
						+ p_150316_1_);
			else {
				p_150316_1_ = p_150316_1_
						.substring(1, p_150316_1_.length() - 1);
				final JsonToNBT.List var2 = new JsonToNBT.List(p_150316_0_);
