...
public class ModelSilverfish extends ModelBase {
	...
	private static final int[][] silverfishBoxLength = new int[][] {
			{ 3, 2, 2 }, { 4, 3, 2 }, { 6, 4, 3 }, { 3, 3, 3 }, { 2, 2, 3 },
			{ 2, 1, 2 }, { 1, 1, 2 } };

	/** The texture positions for the silverfish's model's boxes. */
	private static final int[][] silverfishTexturePositions = new int[][] {
			{ 0, 0 }, { 0, 4 }, { 0, 9 }, { 0, 16 }, { 0, 22 }, { 11, 0 },
			{ 13, 4 } };
	...
	public ModelSilverfish() {
		...
		for (int var2 = 0; var2 < silverfishBodyParts.length; ++var2) {
			silverfishBodyParts[var2] = new ModelRenderer(this,
					silverfishTexturePositions[var2][0],
					silverfishTexturePositions[var2][1]);
			silverfishBodyParts[var2].addBox(silverfishBoxLength[var2][0]
					* -0.5F, 0.0F, silverfishBoxLength[var2][2] * -0.5F,
					silverfishBoxLength[var2][0], silverfishBoxLength[var2][1],
					silverfishBoxLength[var2][2]);
			silverfishBodyParts[var2].setRotationPoint(0.0F,
					24 - silverfishBoxLength[var2][1], var1);
		...
		silverfishWings[0].addBox(-5.0F, 0.0F, silverfishBoxLength[2][2]
		...
		silverfishWings[1].addBox(-3.0F, 0.0F, silverfishBoxLength[4][2]
		...
		silverfishWings[2].addBox(-3.0F, 0.0F, silverfishBoxLength[4][2]
	...
	public void setRotationAngles(float p_78087_1_, float p_78087_2_,
		...
		for (int var8 = 0; var8 < silverfishBodyParts.length; ++var8) {
			silverfishBodyParts[var8].rotateAngleY = MathHelper.cos(p_78087_3_
					* 0.9F + var8 * 0.15F * (float) Math.PI)
					* (float) Math.PI * 0.05F * (1 + Math.abs(var8 - 2));
			silverfishBodyParts[var8].rotationPointX = MathHelper
					.sin(p_78087_3_ * 0.9F + var8 * 0.15F * (float) Math.PI)
					* (float) Math.PI * 0.2F * Math.abs(var8 - 2);
		}
